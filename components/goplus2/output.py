...
# Component-level schema (for the top-level `goplus2:` YAML block)
CONFIG_SCHEMA = cv.Schema({
    cv.GenerateID(): cv.declare_id(GoPlus2Component),
}).extend(i2c.i2c_device_schema(0x38))

# Output-level schema (for `output: - platform: goplus2`)
...
# Register the output platform (2-arg signature expected by ESPHome 2026.x)
output.setup_output_platform_("goplus2", output_to_code)
